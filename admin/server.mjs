...
function gitCalistir(args, timeoutMs = 60000) {
  return new Promise((resolve) => {
    execFile(
      "git",
      args,
      ...
      (err, so, se) => {
        let out = (so || "") + (se || "");
        if (err && err.killed) out += (out ? "\n" : "") + "Zaman aşımı: git " + args.join(" ") + " 60 sn içinde bitmedi.";
        resolve({ ok: !err, out });
      }
    );
  });
...
const ROUTES = {
  ...
  async "POST /api/ihale-ekle"(req, res) {
    ...
    const ic = icerikOku();
    if (!ic[p.no]) ic[p.no] = { pasif: true, dosyalar: [] }; // yeni kart taslak (pasif) başlar
    icerikYaz(ic);
  ...
  async "POST /api/ihale-rapor"(req, res) {
    if (!MK) return json(res, 401, { ok: false, hata: "Önce giriş yapın" });
    const { ikn, rapor } = await body(req);
    const ic = icerikOku();
    if (!ic[ikn]) ic[ikn] = { pasif: false, dosyalar: [] };
    const dizi = (a) => (Array.isArray(a) ? a.map((x) => String(x).trim()).filter(Boolean) : []);
    const r = rapor || {};
    const temiz = {
      ozet: String(r.ozet || "").trim(),
      hususlar: dizi(r.hususlar),
      canAlici: dizi(r.canAlici),
      tavsiyeler: dizi(r.tavsiyeler),
    };
    const bos = !temiz.ozet && !temiz.hususlar.length && !temiz.canAlici.length && !temiz.tavsiyeler.length;
    if (bos) delete ic[ikn].rapor; else ic[ikn].rapor = temiz;
    icerikYaz(ic);
    json(res, 200, { ok: true, ihaleler: buildData() });
  },
